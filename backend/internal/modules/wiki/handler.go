...
func NewWikiHandler(service *Service, uploadsDir string) *WikiHandler {
...
func (h *WikiHandler) ListPages(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "wiki_pages_list_failed"})
		return
	}
...
func (h *WikiHandler) CreatePage(w http.ResponseWriter, r *http.Request) {
	...
	httpx.WriteJSON(w, http.StatusCreated, page)
}
...
func (h *WikiHandler) UploadAttachment(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(maxWikiAttachmentBytes); err != nil {
		httpx.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid_multipart"})
		return
	}

	file, header, err := r.FormFile("file")
	if err != nil {
		httpx.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "file_required"})
		return
	}
	defer file.Close()
	...
	if err != nil {
		httpx.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "file_read_failed"})
		return
	}
	if len(data) > maxWikiAttachmentBytes {
		httpx.WriteJSON(w, http.StatusRequestEntityTooLarge, map[string]string{"error": "file_too_large"})
		return
	}

	contentType, extension, ok := wikiImageType(data, header.Header.Get("Content-Type"))
	if !ok {
		httpx.WriteJSON(w, http.StatusUnsupportedMediaType, map[string]string{"error": "unsupported_image_type"})
		return
	}
	...
	if err != nil {
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "attachment_id_failed"})
		return
	}
	...
	if err := os.MkdirAll(filepath.Dir(absolutePath), 0750); err != nil {
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "attachment_dir_failed"})
		return
	}
	if err := os.WriteFile(absolutePath, data, 0640); err != nil {
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "attachment_write_failed"})
		return
	}

	attachment, err := h.service.CreateAttachment(r.Context(), SaveAttachmentInput{
		ID:           attachmentID,
	...
	httpx.WriteJSON(w, http.StatusCreated, wikiAttachmentPayload(attachment))
}
...
func (h *WikiHandler) AttachmentRaw(w http.ResponseWriter, r *http.Request) {
	...
	if !filepath.IsLocal(relativePath) {
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "invalid_attachment_path"})
		return
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			httpx.WriteJSON(w, http.StatusNotFound, map[string]string{"error": "wiki_attachment_not_found"})
			return
		}
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "wiki_attachment_open_failed"})
		return
	}
	defer file.Close()
	...
	stat, err := file.Stat()
	if err != nil {
		httpx.WriteJSON(w, http.StatusInternalServerError, map[string]string{"error": "wiki_attachment_stat_failed"})
		return
	}
...
func (h *WikiHandler) saveInputFromRequest(w http.ResponseWriter, r *http.Request) (SavePageInput, bool) {
	user, ok := auth.UserFromContext(r.Context())
	if !ok {
		httpx.WriteJSON(w, http.StatusUnauthorized, map[string]string{"error": "unauthorized"})
		return SavePageInput{}, false
	}
	...
	resources := make([]SaveResourceInput, 0, len(payload.Resources))
	for _, resource := range payload.Resources {
		resources = append(resources, SaveResourceInput{
			ID:           resource.ID,
...
func wikiAttachmentPayload(attachment Attachment) wikiAttachmentResponse {
